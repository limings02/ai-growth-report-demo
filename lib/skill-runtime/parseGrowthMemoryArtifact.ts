...
export function parseGrowthMemoryArtifact(
  ...
  try {
    ...
    // JSON 解析失败：fallback 到旧的解析逻辑，包装成 artifact
    console.warn("[parseGrowthMemoryArtifact] JSON parse failed, falling back to parseReportJson");
    ...
    return {
      ...
      sourceTrace: { usedQuestions: [], usedFreeNote: false, missingContext: [], groundingNotes: [] },
      qualityReview: { riskOfFabrication: "medium", emotionalTone: "", weaknesses: [], suggestionsForBetterInput: [] },
    };
  }

  // 解析 report 字段，若缺失也 fallback
  let report;
  if (parsed.report && typeof parsed.report === "object") {
    ...
    // 模型返回了旧格式的 ReportData（没有 artifactVersion）
    report = parseReportJson(raw, childName, reportYear);
  }

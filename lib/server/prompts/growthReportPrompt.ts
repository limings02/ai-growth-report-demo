...
JSON 结构如下：
{
  ...
  "yearlySummary": "3-4 段，段落之间用 \\n\\n（JSON 字符串转义换行符）分隔，基于用户填写内容的年度成长总结",
  "timeline": [
    { "time": "时间描述，如3月、暑假、年底", "title": "简短事件标题", "description": "1-2句事件描述" }
  ],
  "letter": "父母写给孩子的一封信，始终以父母身份写给孩子，风格可以活泼，但视角不变；署名用父母称呼，结尾标注年份",
  "socialPosts": [
...
- yearlySummary：100-150 字，2-3 段，基于真实回答内容
- timeline：3-4 条，从用户的回答中提取真实事件
- letter：100-150 字，温暖克制，对孩子说话的口吻，末尾署名
- socialPosts：3 条，每条 50 字以内，风格不同`;
}
